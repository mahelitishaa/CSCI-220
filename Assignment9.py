...
# [1] Define a function to parse a linear homogeneous recurrence and extract both its
...
def f7(f, n):
    return 3 if n == 0 else (5 if n == 1 else 2 * f(f7, n - 1) + 4 * f(f7, n - 2))
...
def main():
    ...
    recurrences = [
        ...
        ("My Example", "f(0) = 3, f(1) = 5, f(n) = 2f(n-1)+4f(n-2)", f7)
    ]
